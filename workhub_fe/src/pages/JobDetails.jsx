            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              {isLoadingSimilar ? (
                <div className="animate-pulse space-y-4">
                 ...
                 <div className="text-center text-red-500 text-sm">Không thể tải việc làm tương tự.</div>
              ) : similarJobs?.length > 0 ? (
                <div className="space-y-4">
                  {similarJobs.map((similarJob) => (
                    <div key={similarJob.id} className="border-b pb-4 last:border-b-0">
                      <h3 className="font-semibold">
                        <Link to={`/jobs/${similarJob.id}`} className="hover:text-primary">
                          {similarJob.title}
                        </Link>
                      </h3>
                      <p className="text-gray-600 text-sm">{similarJob.recruiter?.companyName || 'N/A'}</p>
                      <div className="flex items-center text-gray-500 mt-2">
                        <span>Địa điểm: {similarJob.location || 'N/A'}</span>
                      </div>
                      <Link
                ...
                <div className="text-center text-gray-500 text-sm">Không tìm thấy việc làm tương tự.</div>
              )}
            </div>
          </div>
        </div>
